...
const MapPage = () => {
  ...
  return (
    <>
      ...
      <main className="pt-[72px] h-screen flex overflow-hidden relative bg-[#090c13]">
        {/* Map column */}
        <div className={cn(
        ...
        <aside className={cn(
          "flex-col w-full lg:w-96 border-l border-white/10 bg-[#0e121d] shrink-0 h-full text-white",
          viewMode === 'list' ? 'flex' : 'hidden lg:flex'
        )}>
          <div className="px-5 py-4 border-b border-white/5 bg-black/20">
            <h1 className="font-sans font-bold text-base uppercase tracking-widest text-[#E28A3E]">Propiedades</h1>
            <p className="text-xs text-slate-400 mt-0.5 font-semibold">
              {isLoading ? 'Cargando…' : `${filtered.length} resultado${filtered.length !== 1 ? 's' : ''}`}
            </p>
          </div>

          <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-24 lg:pb-4 bg-black/10">
            {filtered.map((p) => (
            ...
            {!isLoading && filtered.length === 0 && (
              <div className="text-center py-16">
                <p className="font-sans font-bold text-sm text-slate-400 uppercase tracking-widest">Sin resultados</p>
                <p className="text-xs text-slate-500 mt-2">Ajusta los criterios de búsqueda</p>
              </div>
            )}
          </div>
        </aside>
